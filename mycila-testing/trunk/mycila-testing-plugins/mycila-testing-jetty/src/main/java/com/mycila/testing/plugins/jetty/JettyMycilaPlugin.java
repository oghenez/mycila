...
/**
 * {@link Mycila} testing plugin which load the web application specified by {@link JettyRunWar}
 * using jetty.
 * 
...
public class JettyMycilaPlugin
        ...
        throws Exception
    {
        ...
        if (!testClass.isAnnotationPresent(JettyRunWar.class)) {
            this.logger.debug("skip " + JettyMycilaPlugin.class + " because there is no "
                    + JettyRunWar.class + " annotation on test class");
            return;
        }
        ...
        final FileLocator fallbackFileLocator = (warPath.length() == 0)
                ? new FallbackFileLocator(this.fileLocator, new FixPathFileLocator(
                        new RegFileLocator(),
                        ".*\\.war"))
                : this.fileLocator;
