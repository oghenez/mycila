...
import com.mycila.inject.internal.CglibUtils;
...
final class InvokableFastCtor<T> implements InvokableMember<T> {
    ...
    @Override
    public AnnotatedMemberImpl<Constructor<T>> getMember() {
        return new AnnotatedMemberImpl<Constructor<T>>(ctor.getJavaConstructor());
    }
    ...
    @Override
    public TypeLiteral<T> getMemberType() {
        return TypeLiteral.get(ctor.getJavaConstructor().getDeclaringClass());
    }
