 ...
 */
public final class ExtraScopes {

    private ExtraScopes() {
    }

    public static ScopeWithAnnotation[] values() {
        return new ScopeWithAnnotation[]{
                EXPIRING_SINGLETON,
                RENEWABLE_SINGLETON,
                WEAK_SINGLETON,
                SOFT_SINGLETON,
    ...
    public static final ScopeWithAnnotation EXPIRING_SINGLETON = new MycilaScope(ExpiringSingleton.class) {
        @Override
    ...
    public static final ScopeWithAnnotation RENEWABLE_SINGLETON = new MycilaScope(RenewableSingleton.class) {
        @Override
        public <T> Provider<T> scope(final Key<T> key, final Provider<T> creator) {
            Annotation annotation = key.getAnnotation();
            if (!Expirity.class.isInstance(annotation))
    ...
    public static final ScopeWithAnnotation WEAK_SINGLETON = new RefScope(WeakSingleton.class) {
        @Override
    ...
    public static final ScopeWithAnnotation SOFT_SINGLETON = new RefScope(SoftSingleton.class) {
        @Override
        protected <T> Reference<T> build(T instance) {
            return new WeakReference<T>(instance);
        }
    ...
    public static final ScopeWithAnnotation CONCURRENT_SINGLETON = new MycilaScope(ConcurrentSingleton.class) {
        ...
        private final long timeout = TimeUnit.SECONDS.toNanos(10);
    ...
    public static void installAll(Binder binder) {
        for (ScopeWithAnnotation mycilaScope : ExtraScopes.values()) {
            binder.bindScope(mycilaScope.getScopeAnnotation(), mycilaScope);
            binder.requestInjection(mycilaScope);
        }
    }
    ...
    private static abstract class MycilaScope implements ScopeWithAnnotation {
        ...
        public final String toString() {
            return ExtraScopes.class.getSimpleName() + "." + annotationScope.getSimpleName();
        }
