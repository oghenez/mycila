...
public class ModMatrix {
  ...
  public ModMatrix(int r,int c,BigInteger[] a, BigInteger m) {
         ...
         for (int i=0;i<r;i++) {
             for (int j=0;j<c;j++) {
                 ...
                 if (pos<a.length&&a[pos]!=null) array[i+1][j+1]=BigIntegerMath.lnr(a[pos],m);
  ...
  public ModMatrix add(ModMatrix m) throws MatricesNonConformableException {
         ...
         } else throw new MatricesNonConformableException("These matrices cannot be added; different dimensions.");
         return result;
  }
  ...
  public ModMatrix subtract(ModMatrix m) throws MatricesNonConformableException {
     //Multiply the 2nd matrix by the scalar -1 then add them-see multiply(BigInteger) method
  ...
  public ModMatrix multiply(ModMatrix m) throws MatricesNonConformableException {
         ...
         if (!modulus.equals(m.modulus)) throw new MatricesNonConformableException("These matrices cannot be multiplied; different moduli.");
                  ...
                  for (int i=1;i<=numRows;i++) {
                      ...
                      for (int j=1;j<=m.numCols;j++) {
                          ...
                          for (int k=1;k<=m.numRows;k++)
                             result.array[i][j]=BigIntegerMath.lnr(result.array[i][j].add(array[i][k].multiply(m.array[k][j])),modulus);
                      }
                  }
         } else throw new MatricesNonConformableException("These matrices cannot be multiplied!");
         return result;
  }
