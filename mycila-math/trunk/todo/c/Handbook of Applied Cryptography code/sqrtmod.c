   ...
   if it exists 0 otherwise */
{
  ...
  for (i = 1; i < s; i++) {
    ...
    /* Krystian Szczesny Izycki [K.Izycki@elka.pw.edu.pl]
       discovered the bug in the following line of code
       on or about January 8, 2004
    if (d == p - 1) r = r * c % p; */
    /* modification January 10, 2004 */
    if (d == - 1) r = r * c % p;
    c = c * c % p;
  }
  return r;
